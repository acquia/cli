...
final class PushFilesCommand extends PushCommandBase
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->rsyncFilesToCloud($siteInstance, $this->getOutputCallback($output, $this->checklist), $chosenSite);
        $this->checklist->completePreviousItem();

        return Command::SUCCESS;
    }

    private function rsyncFilesToCloud(SiteInstanceResponse $siteInstance, ?callable $outputCallback = null, ?string $site = null): void
    {
        $sourceDir = $this->getLocalFilesDir($site);
        $destinationDir = $siteInstance->environment->codebase->vcs_url . ':' . $this->getCloudFilesDir($siteInstance);
