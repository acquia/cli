...
final class PushDatabaseCommand extends PushCommandBase
{
    ...
    private function importDatabaseDumpOnRemote(EnvironmentResponse $environment, string $remoteDumpFilepath, DatabaseResponse $database): void
    {
        ...
        $command = "pv $remoteDumpFilepath --bytes --rate | gunzip | MYSQL_PWD=" . escapeshellarg($database->password) . " mysql --host={$this->getHostFromDatabaseResponse($environment, $database)} --user=$database->user_name {$this->getNameFromDatabaseResponse($database)}";
