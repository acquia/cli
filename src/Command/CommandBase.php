...
abstract class CommandBase extends Command implements LoggerAwareInterface {
  ...
  protected function promptLinkApplication(
    ...
    $question = new ConfirmationQuestion("<question>Would you like to link the Cloud application <options=bold>{$cloud_application->name}</> to this repository</question>? ");
    $helper = $this->getHelper('question');
    $answer = $helper->ask($this->input, $this->output, $question);
    if ($answer) {
