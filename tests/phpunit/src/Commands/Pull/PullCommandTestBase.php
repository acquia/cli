...
abstract class PullCommandTestBase extends CommandTestBase
{
    ...
    protected function mockExecuteMySqlImport(
        ...
        $command = $pvExists
            ? 'bash -c "set -o pipefail; pv \"${:LOCAL_DUMP_FILEPATH}\" --bytes --rate | gunzip | MYSQL_PWD=\"${:MYSQL_PASSWORD}\" mysql --host=\"${:MYSQL_HOST}\" --user=\"${:MYSQL_USER}\" \"${:MYSQL_DATABASE}\""'
            : 'bash -c "set -o pipefail; gunzip -c \"${:LOCAL_DUMP_FILEPATH}\" | MYSQL_PWD=\"${:MYSQL_PASSWORD}\" mysql --host=\"${:MYSQL_HOST}\" --user=\"${:MYSQL_USER}\" \"${:MYSQL_DATABASE}\""';
        $expectedEnv = [
