...
Note: on certain systems or PHP builds where the `php-phar` extension is not installed, you might not be able to run `acli`. Make sure to install/compile with the extension if PHP returns `Class 'Phar' not found`.
